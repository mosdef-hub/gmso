...
def _base_doc_to_sections(base_doc: str) -> dict:
    """Convert base doc to the sections expected in numpydoc style."""
    ...
    # Hack but works
    doc_lines = [
        f"\t{doc_lines[j]}" if leading_spaces[j] > 0 else doc_lines[j]
    ...
    sections.update({"Summary": summary, "Extended Summary": extended_summary})
    return sections


def _inject_parameters_from_fields(
    fields: Dict[str, Any],
    by_alias: bool = True,
    ...
    for key, value in fields.items():
        ...
        parameters.append(
            f"{name} : " f"{_infer_type(value.type_)}" f", default={default}"
        )
        if init_signature:
    ...
    if name_map:
        parameters = parameters.replace(
            name_map[1].lower(), name_map[0].lower()
        )
    if init_signature:
...
def _find_extra_params(cls, fields, init_params):
    ...
    for param in init_params:
        if param not in aliases_set and param != "self":
            if hasattr(cls, param) and isinstance(
                getattr(cls, param), property
            ):
...
class AutoDocGenerator:
    """Generate __doc__ attribute for pydantic base model classes and descendants.
        ...
        if target is BaseModel or issubclass(target, BaseModel):
            if hasattr(target, BASE_DOC_ATTR):
                ...
                raise AttributeError(
                    f"No matching attribute {BASE_DOC_ATTR} found in {target.__name__}"
                )

        else:
            raise TypeError(
                "Cannot generate documentation for non-basemodel descendants"
            )
    ...
    def _should_inject_init(self) -> bool:
        """Inject __init__ docs if super and self are the same."""
    ...
    def _name_map(self, map_names: bool = True) -> Union[Tuple[str, str], None]:
        """Return mapping between super and this object."""
        super_classes = self.target.mro()
        if map_names and len(super_classes) > 2:
            return (self.target.__name__, super_classes[1].__name__)
        else:
            return None
    ...
    def __del__(self) -> None:
        """Define behavior when the instance is about to be destroyed."""
...
def apply_docs(
    target_class: Type[BaseModel], map_names: bool = True, silent: bool = True
) -> None:
    ...
    try:
        target_class.__doc__ = AutoDocGenerator(
            target_class, map_names=map_names
        )
