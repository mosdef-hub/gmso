...
<<<<<<< HEAD
    ...
    for i in range(ndih):
        ...
        req=d_phi[i]
        n=d_n[i]
        diehdrals.dihedral_type=DihedralType(name="HarmonicTorsionPotential",
                                expression="k * (1 + cos(n * phi - phi_eq))**2",
                                independent_variables={"phi"},
                                member_types=(site_1.atom_type.name,site_2.atom_type.name,site_3.atom_type.name,site_4.atom_type.name),
                                member_classes=(site_1.atom_type.atomclass,site_2.atom_type.atomclass,site_3.atom_type.atomclass,site_4.atom_type.atomclass),
                                parameters={
                                "k": K * u.Unit("kJ / (deg**2)"),
                                "phi_eq": 180 * u.deg,
                                "n":n * u.dimensionless})
