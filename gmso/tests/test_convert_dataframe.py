...
from gmso.external.convert_dataframe import to_dataframeDict, _recursive_getattr, multi_topology_dataframe
...
class TestConvertDataFrame(BaseTest):
    ...
    def test_recursive_dihedrals(self, typed_ethane):
        out = list(_recursive_getattr(typed_ethane, "dihedrals", "dihedral_type.member_types"))
        expected = [dihedral.dihedral_type.member_types for dihedral in typed_ethane.dihedrals]
        assert out == expected
    ...
    def test_to_dataframeDict(self, typed_ethane):
        ...
        for parameter, val in zip(checkList, expected_valuesList):
            assert len(to_dataframeDict(typed_ethane, parameter=parameter)[parameter]) == val
    ...
    def test_dataframe_impropers(self, benzeneTopology):
        ...
        for parameter, val in zip(checkList, expected_valuesList):
            assert len(to_dataframeDict(benzeneTopology, parameter=parameter)[parameter]) == val
    ...
    def test_dataframe_default_columns(self, typed_ethane):
        expected_columns = ["name", "atom_type.name", "sigma", "epsilon", "charge", "mass"]
        assert np.all(list(to_dataframeDict(typed_ethane, "sites")["sites"].columns) == expected_columns)
    ...
    def test_dataframe_specified_columns(self, typed_ethane):
        ...
        assert np.all(
            list(
                to_dataframeDict(
                    typed_ethane, "sites", columns=input_columns, format="specific_columns"
                )["sites"].columns) 
            == expected_columns
            )
    ...
    def test_dataframe_publication(self, benzeneTopology):
        dfDict = to_dataframeDict(
            benzeneTopology, "all", format="publication"
        )
        df = dfDict["sites"]
    ...
    def test_dataframe_remove_duplicates(self, benzeneTopology):
        ...
        checkList = [2, 2, 3, 1]
        for connect, check in zip(connectList, checkList):
            dfDict = to_dataframeDict(benzeneTopology, connect, format="remove_duplicates")
            df = dfDict[connect]
            assert len(df.index) == check 
    ...
    def test_multi_topology_dataframe(self, benzeneTopology, spce_water):
        dfDict = multi_topology_dataframe([benzeneTopology, spce_water, benzeneTopology])
